...
async function generateOne({ entry, outRel, label }) {
  ...
  const outDir = path.join(srcDir, outRel);
...
function writeGenWrappers({ name, outRel }) {
  ...
  const genDir = path.join(srcDir, outRel);
  ...
  fs.writeFileSync(
    ...
    `import type { ResponseType } from 'axios';\n` +
      ...
      `import { createOpenApiInitializer, resolveBase, type OpenApiCommonInit } from '../request';\n\n` +
