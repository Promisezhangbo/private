...
1. **构建**：**`pnpm exec turbo run build --filter=<scope>`**（与 CI 一致），产物在 **`dist/<scope>/`**。`turbo.json` 里 **`build` → `dependsOn: ["^generate", ...]`** 会在构建前对 workspace 依赖执行 **`generate`**；**`packages/openapi/gen/`** 在 **`.gitignore`** 中，检出后必须生成，否则 `tsc` 会报找不到 **`../gen/clients`**、**`@packages/openapi/*-gen-types`** 等。若误用 **`pnpm --filter <app> build`**，会绕过 Turbo，**不会**跑 openapi 的 **`generate`**，从而报错。
