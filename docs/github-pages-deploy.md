...
| 6 | Build(单应用) | `scope != all`：**`pnpm exec turbo run build --filter=<scope>`**（同上 env；会先执行依赖的 **`^generate`**，如 **`@packages/openapi`**；勿用 `pnpm --filter` 直连 build，否则会缺 `gen/`）。 |
...
1. **构建**：**`pnpm exec turbo run build --filter=<scope>`**（与 CI 一致），产物在 **`dist/<scope>/`**。`turbo.json` 里 **`build` → `dependsOn: ["^generate", ...]`** 会在构建前对 workspace 依赖执行 **`generate`**；**`packages/openapi/gen/`** 在 **`.gitignore`** 中，检出后必须生成，否则 `tsc` 会报找不到 **`gen/index.ts`**、**`@packages/openapi/*-gen-types`** 等。若误用 **`pnpm --filter <app> build`**，会绕过 Turbo，**不会**跑 openapi 的 **`generate`**，从而报错。
...
DEPLOY_SCOPE=skill BUILD_BRANCH=$(git branch --show-current) pnpm exec turbo run build --filter=skill
